...
/*
 * This file is a copy of {@see MongoDB\GridFS\CollectionWrapper}
 *
 * Changes: ensureFilesIndex method - set Unique index on filename
 *
...
class CollectionWrapper
{
    ...
    private function ensureFilesIndex()
    {
        foreach ($this->filesCollection->listIndexes() as $index) {
            if ($index->isUnique() && $index->getKey() === ['filename' => 1]) {
                return;
            }
        }
        $this->filesCollection->createIndex(['filename' => 1], ['unique' => true]);
    }
