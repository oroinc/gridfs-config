...
/*
 * This file is a copy of {@see MongoDB\GridFS\Bucket}
 *
 ...
 * collections.
 *
 * This bucket uses Oro\Bundle\GridFSConfigBundle\GridFS\CollectionWrapper.
 *
 * @api
 * @SuppressWarnings(PHPMD.ExcessiveClassComplexity)
 * @SuppressWarnings(PHPMD.TooManyPublicMethods)
 */
class Bucket extends \MongoDB\GridFS\Bucket
{
    /** @var string */
    ...
    public function __construct(Manager $manager, $databaseName, array $options = [])
    {
        ...
        if ($options['chunkSizeBytes'] < 1) {
            throw new InvalidArgumentException(
                sprintf('Expected "chunkSizeBytes" option to be >= 1, %d given', $options['chunkSizeBytes'])
            );
        }
        ...
        $this->readConcern = isset($options['readConcern'])
            ? $options['readConcern']
            : $this->manager->getReadConcern()
        ;
        ...
        $collectionOptions = array_intersect_key(
            $options,
            ['readConcern' => 1, 'readPreference' => 1, 'typeMap' => 1, 'writeConcern' => 1]
        );
     ...
     */
    #[\Override]
    public function downloadToStreamByName($filename, $destination, array $options = []): void
    {
        if (!is_resource($destination) || get_resource_type($destination) != "stream") {
            throw InvalidArgumentException::invalidType('$destination', $destination, 'resource');
        }
     ...
     */
    #[\Override]
    public function rename($id, $newFilename): void
    {
     ...
     */
    #[\Override]
    public function uploadFromStream($filename, $source, array $options = []): mixed
    {
    ...
    private function getRawFileDocumentForStream($stream)
    {
        ...
        if (!isset($metadata['wrapper_data']) || !$metadata['wrapper_data'] instanceof StreamWrapper) {
            throw InvalidArgumentException::invalidType(
                '$stream wrapper data',
                isset($metadata['wrapper_data']) ? $metadata['wrapper_data'] : null,
                StreamWrapper::class
            );
        }
